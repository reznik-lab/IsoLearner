### This IsoLearner is for conducting a GridSearch, not training models
...
class IsoLearner:
    def __init__(self, absolute_data_path, relative_data_path, morans_path = 'valid-metabs-brain-glucose.txt', tracer = 'glucose', FML = True, num_replicates = 6, morans_cutoff = 0.75):
        '''
        ...
        self.all_models = []
        #self.all_models_np = np.empty([0, [0, 0], [0, 0], [0, 0]])
    ...
    def training(self, feature_data, target_data, model_to_use, checkpoint_path = './saved-weights/KD-M1-unranked-dropout/checkpoint', train = True, TRAIN_ENTIRE_BRAIN = False, ranked = True, EPOCHS = 100, BATCH_SIZE = 32):
        print("HI")
        ...
        if train:
            ...
            history = model.fit(x_train, y_train, batch_size = BATCH_SIZE, verbose=1, validation_data = (x_val, y_val), epochs=EPOCHS, callbacks=[model_checkpoint_callback, early_callback])
    ...
    def cross_validation_training(self, checkpoints_dir_label = "glucose", checkpoints_path = "./saved-weights", EPOCHS = 30):
        '''
        ...
        training_features, training_targets, testing_features, testing_targets = self.create_full_dataset(self.clean_ion_data, self.clean_iso_data, holdout=True, holdout_index=0)
        print(f"Training with replicate 0 heldout. # samples = {training_features.shape[0]}")
        
        header = ['model', 'training loss', 'testing loss', 'training mse', 'testing mse', 'training mae', 'testing mae']
        
        with open('results-round-2.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(header) # Use writerow for single list

        # Train the model 
        for i in range(len(os.listdir('/Users/goldfei/Documents/Iso2/round-2-models'))):

            checkpoint = (f'holdout-0')
            print(i+30)
            os.mkdir(f'{checkpoints_path}/cross-validation-{checkpoints_dir_label}/{checkpoint}')
            choose_model = load_model(f'/Users/goldfei/Documents/Iso2/round-2-models/model-{i+30}.h5')
            print(choose_model)
            history = self.training(training_features, training_targets, choose_model, f'{full_checkpoint_dir}/{checkpoint}/checkpoint', train = True, TRAIN_ENTIRE_BRAIN = True, EPOCHS = EPOCHS, BATCH_SIZE = 128)
            ...
            self.all_models.append([choose_model, training_loss[-1], test_loss[-1], training_mse[-1], test_mse[-1], training_mae[-1], test_mae[-1]])
            model_results = [choose_model, training_loss[-1], test_loss[-1], training_mse[-1], test_mse[-1], training_mae[-1], test_mae[-1]]
            #self.all_models.np.append([choose_model, [training_loss, test_loss], [training_mse, test_mse], [training_mae, test_mae]])
            #self.all_models_np = np.array(self.all_models)
            ...
            # plt.title(f'Holdout: replicate 0')
            # plt.xlabel('Epoch')
            # plt.ylabel('Loss')
            # plt.ylim([0, 10])
            # plt.show();
            
            config = choose_model.get_config()
            
            nodes_layer_1 = config['layers'][1]['config']['units']
            activation_layer_1 = config['layers'][1]['config']['activation']
            nodes_layer_2 = config['layers'][3]['config']['units']
            activation_layer_2 = config['layers'][3]['config']['activation']
            nodes_layer_3 = config['layers'][5]['config']['units']
            activation_layer_3 = config['layers'][5]['config']['activation']
            dropout_1 = config['layers'][7]['config']['rate']
            nodes_layer_4 = config['layers'][8]['config']['units']
            activation_layer_4 = config['layers'][8]['config']['activation']
            nodes_layer_5 = config['layers'][10]['config']['units']
            activation_layer_5 = config['layers'][10]['config']['activation']
            nodes_layer_6 = config['layers'][12]['config']['units']
            activation_layer_6 = config['layers'][12]['config']['activation']
            dropout_2 = config['layers'][14]['config']['rate']
            nodes_layer_7 = config['layers'][15]['config']['units']
            activation_layer_7 = config['layers'][15]['config']['activation']
            nodes_layer_final = config['layers'][17]['config']['units']
            activation_layer_final = config['layers'][17]['config']['activation']
            ...
            with open(f'./model-configs-round-2/model-{i+30}.json', 'w', encoding='utf-8') as f:
                json.dump(model_data, f, ensure_ascii=False, indent=4)
                
            with open('results-round-2.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(model_results) # Use writerow for single list
                

            shutil.rmtree(f'{checkpoints_path}/cross-validation-{checkpoints_dir_label}/{checkpoint}')
