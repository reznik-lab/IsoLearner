...
class IsoLearner:
    ...
    def model_builder_keras(hp):
        ...
        # lambda_val = 0.01
        num_ion_counts = 50
        num_isotopolouges = 654

        hp_layer_1 = hp.Int('layer_1', min_value = 128, max_value = 512, step = 128)
        #hp_layer_2 = hp.Int('layer_2', min_value = 128, max_value = 512, step = 128)
        #hp_layer_3 = hp.Int('layer_3', min_value = 128, max_value = 512, step = 128)
        #hp_layer_4 = hp.Int('layer_4', min_value = 128, max_value = 512, step = 128)
        #hp_layer_5 = hp.Int('layer_5', min_value = 128, max_value = 512, step = 128)
        #hp_layer_6 = hp.Int('layer_6', min_value = 128, max_value = 512, step = 128)
        #hp_layer_7 = hp.int('layer_7', min_value = 1, max_value = 1000, step = 100)

        # hp_learning_rate = hp.Choice('learning_rate', values = [3e-05, 0.001, 0.01])

        #hp_dropout = hp.Choice('dropout', values = [0.25, 0.5])

        model.add(tf.keras.layers.Dense(units=hp_layer_1, input_dim = num_ion_counts, kernel_initializer='he_uniform', activation=hp_activation, kernel_regularizer=l2(lambda_val))),
        BatchNormalization()

        model.add(tf.keras.layers.Dense(units=128, kernel_initializer='he_uniform', activation=hp_activation, kernel_regularizer=l2(lambda_val))),
        BatchNormalization()

        model.add(tf.keras.layers.Dense(units=256, kernel_initializer='he_uniform', activation=hp_activation, kernel_regularizer=l2(lambda_val))),
        BatchNormalization(),
        Dropout(0.25)
        ...
        model.add(tf.keras.layers.Dense(units = num_isotopolouges, kernel_initializer='he_uniform', activation=hp_activation, kernel_regularizer=l2(lambda_val))),
        BatchNormalization()
    ...
    def training(self, feature_data, target_data, checkpoint_path = './saved-weights/KD-M1-unranked-dropout/checkpoint', train = True, TRAIN_ENTIRE_BRAIN = False, ranked = True, EPOCHS = 100, BATCH_SIZE = 32):
        ...
        #model = self.model_builder(num_ion_counts, num_isotopolouges, 0.01)
    ...
    def search_training(self, feature_data, target_data, checkpoint_path = './saved-weights/KD-M1-unranked-dropout/checkpoint', train = True, TRAIN_ENTIRE_BRAIN = False, ranked = True, EPOCHS = 100, BATCH_SIZE = 32):
        print("HI")
        # Whether or not to treat the dataset as training data
        if TRAIN_ENTIRE_BRAIN:
            num_ion_counts, num_isotopolouges, x_train, y_train, x_val, y_val = self.preprocess_data(feature_data, target_data, testing_split = False)
        ...
        # fit model
        if train:
            tuner = kt.Hyperband(model,objective='mae',max_epochs=100, factor=3, directory= 'dir', project_name='x')
            stop_early = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=3)
            #history = model.fit(x_train, y_train, batch_size = BATCH_SIZE, verbose=1, validation_data = (x_val, y_val), epochs=EPOCHS, callbacks=[model_checkpoint_callback])
            history = tuner.search(x_train, y_train, validation_split=0.2, callbacks=[stop_early, model_checkpoint_callback], batch_size = BATCH_SIZE, verbose=1, validation_data = (x_val, y_val), epochs=EPOCHS)
            best_hps = tuner.get_best_hyperparameters(num_trials=1)[0]
            print(f'Best Params:  {best_hps}')
    ...
    def cross_validation_search(self, checkpoints_dir_label = 'glucose', checkpoints_path = "./saved-weights", EPOCHS = 100):
        '''
        Performs a GridSearch for one holdout replicate. The saved weights for each model checkpoint is saved in the directory 
        ...
        Parameters:
            ...
            - checkpoints_path (string): path to directory in which to house the checkpoint directory
        '''
        print("WE ARE IN THE GRIDSEARCH FUNCTION")    
        ...
        # Create training and testing sets - pull one replicate out of the set 
        training_features, training_targets, testing_features, testing_targets = self.create_full_dataset(self.clean_ion_data, self.clean_iso_data, holdout_index = 0)
        print(f"Training with replicate 0 heldout. # samples = {training_features.shape[0]}")

        # Train the model 
        checkpoint = f'holdout-0'
        os.mkdir(f'{checkpoints_path}/cross-validation-{checkpoints_dir_label}/{checkpoint}')
        history = self.search_training(training_features, training_targets, f'{full_checkpoint_dir}/{checkpoint}/checkpoint', train = True, TRAIN_ENTIRE_BRAIN = False, EPOCHS = EPOCHS, BATCH_SIZE = 128)
